...
contract SPCTreasury is Ownable {
    ...
    address public SPICY;
    uint public SPICYQueue; // Delays change to SPICY address
    ...
    //    isLiquidityToken[ _SPCDAI ] = true;
    //    liquidityTokens.push( _SPCDAI );

        secondsNeededForQueue = _secondsNeededForQueue;
    }

    /**
        @notice allow approved address to deposit an asset for SPC
        @param _amount uint
    ...
    function deposit( uint _amount, address _token, uint _profit ) external returns ( uint send_ ) {
        ...
        // mint SPC needed and store amount of rewards for distribution
        send_ = value.sub( _profit );
        IERC20Mintable( SPC ).mint( msg.sender, send_ );
    ...
    /**
        @notice allow approved address to burn SPC for reserves
        @param _amount uint
        @param _token address
     */
    function withdraw( uint _amount, address _token ) external {
        ...
        uint value = valueOf( _token, _amount );
        ISPC( SPC ).burnFrom( msg.sender, value );
    ...
    function incurDebt( uint _amount, address _token ) external {
        ...
        uint maximumDebt = IERC20( SPICY ).balanceOf( msg.sender ); // Can only borrow against SPICY held
    ...
    /**
        @notice allow approved address to repay borrowed reserves with SPC
        @param _amount uint
     */
    function repayDebtWithSPC( uint _amount ) external {
        require( isDebtor[ msg.sender ], "Not approved" );

        ISPC( SPC ).burnFrom( msg.sender, _amount );

        debtorBalance[ msg.sender ] = debtorBalance[ msg.sender ].sub( _amount );
        totalDebt = totalDebt.sub( _amount );

        emit RepayDebt( msg.sender, SPC, _amount, _amount );
    }
    ...
    function mintRewards( address _recipient, uint _amount ) external {
        ...
        IERC20Mintable( SPC ).mint( _recipient, _amount );
    ...
    function excessReserves() public view returns ( uint ) {
        return totalReserves.sub( IERC20( SPC ).totalSupply().sub( totalDebt ) );
    }
    ...
    /**
        @notice returns SPC valuation of asset
        @param _token address
        @param _amount uint
        @return value_ uint
     */
    function valueOf( address _token, uint _amount ) public view returns ( uint value_ ) {
        if ( isReserveToken[ _token ] ) {
            // convert amount to match SPC decimals
            value_ = _amount.mul( 10 ** IERC20( SPC ).decimals() ).div( 10 ** IERC20( _token ).decimals() );
    ...
    function queue( MANAGING _managing, address _address ) external onlyManager() returns ( bool ) {
        ...
        } else if ( _managing == MANAGING.SPICY ) { // 9
            SPICYQueue = uint32(block.timestamp).add32( secondsNeededForQueue );
        } else return false;
