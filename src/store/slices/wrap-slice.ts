...
import { wsVerseTokenContract } from "../../abi";
...
export const changeApproval = createAsyncThunk("wrapping/changeApproval", async ({ provider, address, networkID }: IChangeApproval, { dispatch }) => {
    ...
    const sverseContract = new ethers.Contract(addresses.SPICY_ADDRESS, wsVerseTokenContract, signer);

    let approveTx;
    try {
        const gasPrice = await getGasPrice(provider);

        approveTx = await sverseContract.approve(addresses.WSPICY_ADDRESS, ethers.constants.MaxUint256, { gasPrice });
    ...
    const wsverseAllowance = await sverseContract.allowance(address, addresses.WSPICY_ADDRESS);

    return dispatch(
        fetchAccountSuccess({
            wrapping: {
                wsverse: Number(wsverseAllowance),
            },
        }),
    );
});
...
export const changeWrap = createAsyncThunk("wrapping/changeWrap", async ({ isWrap, value, provider, networkID, address }: IChangeWrap, { dispatch }) => {
    ...
    const wsverseContract = new ethers.Contract(addresses.WSPICY_ADDRESS, wsVerseTokenContract, signer);
    ...
    try {
        ...
        if (isWrap) {
            wrapTx = await wsverseContract.wrap(amountInWei, { gasPrice });
        } else {
            wrapTx = await wsverseContract.unwrap(amountInWei, { gasPrice });
        }
...
const calcWrapValue = async ({ isWrap, value, provider, networkID }: IWrapDetails): Promise<number> => {
    ...
    if (isWrap) {
        const wsverseValue = await wsverseContract.MEMOTowMEMO(amountInWei);
        wrapValue = wsverseValue / Math.pow(10, 18);
    } else {
        const sverseValue = await wsverseContract.wMEMOToMEMO(amountInWei);
        wrapValue = sverseValue / Math.pow(10, 9);
    }
